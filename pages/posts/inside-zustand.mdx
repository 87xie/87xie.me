...
tag: react, zustand, state management
date: 2024-06-17
---
...
- 透過 `Set<Listener>` 來存放 listener callback
...
先看到 `useStore`，是透過 react 的 [`useSyncExternalStore`](https://react.dev/reference/react/useSyncExternalStore#subscribing-to-an-external-store) 整合 vanilla store：
...
再回到完整程式碼的 `createImpl`，這裡整理一下目前已知：

- `api` 是 vanilla store 的介面
- `api` 會被複製到 `useBoundStore` 上（透過 `Object.assign`）
- `useBoundStore` 是接收 `selector` 參數的 react hook，`selector` 是用來選取 vanilla store state 的 callback
...
從上述結果得知 `useBoundStore` 是基於 `useSyncExternalStore` 實現，當調用 `useBoundStore(selector){:js}`：
