...
知道可能與 `Query` 相關後，從 `Query` 的程式碼得知：

- `Query` 為 `Removable` 的子類別 (L6, L9)
...
- 透過 `setTimeout` 設置 garbage collected 的定時器 (L15-17)
- 在 `updateGcTime` (L23-L26) 對應官方文件提到：
    - client-side `gcTime` 預設 5 minutes
    - server-side `gcTime` 預設 `Infinity`
...
- TanStack Query 是透過 `setTimeout`，定時檢查 `Query`, `Mutation` 實例的狀態，若判定為未使用，則向 `QueryCache` 或 `MutationCache` 清除該實例。`gcTime` 即為 `setTimeout` 的 delay ms
- 如果 `Query` 的請求進行中，或正被 `QueryObserver` 訂閱，該實例不會被清除
- 如果 `Mutation` 的請求進行中，或正被 `MutationObserver` 訂閱，該實例不會被清除
- `Query`, `Mutation` 會在不同時機，設置 garbage collection 定時器，例如：
    - 建立 `Query`, `Mutation` 實例時
    - `Query` 執行請求後，收到失敗或成功回覆時
