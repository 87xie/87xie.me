...
- `mergeSort` 將陣列對半切分成兩個小陣列，直到每個小陣列中只剩一個元素，將結果做為參數傳至 `merge`
- `merge` 比較兩個陣列第一個元素，較小的元素優先放入合併陣列
- 拆分共 n - 1 次，合併需要 n 次，共 log n 輪， `(n - 1) + (n log n)` 忽略常數取最高次方，時間複雜度為 `O(n log n)`
...
1. 建立兩個陣列分別為 `left`、`right`，比較陣列剩餘元素，比 `pivot` 小則放至 `left`，反之放至 `right`
1. 將 `left`、`right` 重複 1、2 步驟，直到無法再劃分。

其原理像是 binary search tree 中的 inorder traversal，先從左邊的子節點開始取值，可以確保順序是由小到大。根據判斷基準點 `pivot` 的實作方式，會影響 quick sort 的時間複雜度，如同 binary search tree 不同樹高對搜尋結果的影響，下方範例最差情況為 `O(n²)`，最佳情況為 `O(n log n)`
