...
slug: review-of-the-book-mastering-modular-javascript
date: 2022-09-02
tags:
  - javascript
  - book-review
...
這裡主要依照自己的經驗紀錄讀後心得，所以跟書中提到的內容有些許出入。
...
對應到前端開發，假設一個名為 `user-profile` 的 component 需要處理

- ui style
- data fetching

根據上述兩個項目，我們可以先依據不同性質將檔案劃分，其目錄結構如下

```bash
user-profile/
  index.js              # file entry point
  user-profile.jsx      # component
  user-profile.style.js # for ui styles
  user-profile.query.js # for data fetching
```

透過不同檔案的以及公開介面，我們把部分的實作細節隱藏，並將樣式及實作細節做關注點分離

```jsx
import * as Styled from './user-profile.style.js';
import { useUserQuery } from './user-profile.query.js';
...
const UserProfile = () => {
  ...
  if (isLoading) return <Styled.Spinner />;
  if (isError) return (
    <Styled.ErrorMessage>
...
當收到調整 data fetching 的 api 需求時，你可以更快在 `user-profile.query.js` 的 lexical scope 中，找出此次需要變更的範圍，這也有助於 code review 的進行

測試方面，不管是使用 api mocking 或是針對 `useUserQuery` hook 的 return value 進行 mock 都是可行的做法 (個人偏好前者)

當 component 知道越少實作細節，在後續維護或是重構上會更輕鬆，以 data fetching 為例，你可能會使用到的三方套件有

- axios / fetch (native)
- react-query / swr

透過 SRP 切分業務職責，並維持相同的介面，你都可以立刻更換相關的三方套件或是細節實作
