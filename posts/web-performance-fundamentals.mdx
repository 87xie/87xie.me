...
綜觀來說前端只是網頁的一部分，但是面對效能議題，前端往往是第一線的處理人員，就像急診室的值班醫師，首先我們要視病人情況，做出對應處理，像是大量出血的病患需要立即止血；呼吸短促的病患需要給予人工呼吸器，我們盡可能地排除當下急重症，後續才能依照病情進行相關的轉診手續。

回到技術面，即使面對無法立即處理的效能問題，我們也可以從其他方面下手，例如透過 UI 向用戶指示，某個區塊正在載入某種資料（圖片、文字等 ... ），同時避免資料載入後造成非預期的版面位移，或是把較耗時的 API 、工作，移至不同階段處理 (Minimize main thread work)，提升用戶第一次與網頁互動的時間 (FID)，透過各種方法來分散用戶等待時的注意力，都是可能提升用戶感知效能的方式之一。

為什麼要重視效能，這裡不談高大上的理論像是

- 載入速度每減少 1 秒，轉換率 (CVR) 提升 2%
- 載入速度每減少 100 毫秒，營收提升 1%

第一點是無法直觀從數據看出效能優化，對轉換率與營收之間有什麼直接的因果關係，我相信各位也有遇過效能未改善，但營收或轉換率上升的情況。

第二點是這些數據為市面上大公司的統計成果，未必適用在你目前負責的產品上。若有實際收集的 field data 與 business data 分析後的數據，可能更適合作為此產品商業決策上的依據，但也不是每間公司都有 data team 或是 growth team 這種資源。
...
這裡主要記錄一些 Web 前端上的效能指標及優化的大方向，參考的相關連結會附在文末。

## Perceived Performance

用戶對網站速度的主觀感受的指標

因為著重在使用者體驗，我們可以根據下列幾項原則，進一步分散用戶在等待時的注意力，提升感知的載入時間
